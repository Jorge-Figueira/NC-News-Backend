...
exports.fetchArticlesById = (requested_id) => {
  return db
    .query("SELECT * FROM comments WHERE article_id = $1;", [requested_id])
    .then(({ rows }) => {
      if (rows.length === 0) {
        return db
        ...
        return db
          .query(
            "SELECT articles.*, COUNT(*) AS comment_count FROM articles  JOIN comments ON articles.article_id = comments.article_id WHERE articles.article_id = $1 GROUP BY articles.article_id;",
            [requested_id],
          )
          .then(({ rows }) => {
            return rows[0];
...
exports.writeComment = ({ body, params }) => {
  return db
    .query(
      "INSERT INTO comments  (body, author, article_id, votes, created_at) VALUES ($1, $2, $3, 0, CURRENT_TIMESTAMP) RETURNING *;",
      [body.body, body.username, params.article_id],
    )
    .then(({ rows }) => {
      return rows[0];
    })
    .catch((error) => {
      if (error.code === "23503") {
        ...
        return Promise.reject({
          status: 404,
          msg: `${columnWithError[0]} not found`,
        });
      } else {
